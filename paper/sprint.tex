...
\begin{abstract}
\noindent
Preference-coherence fits a Thurstonian model to pairwise choices. High
held-out accuracy is read as evidence of emergent values. We add the missing
control: the same battery with every referent replaced by an invented word.
Prompt, pair set, fit and metric are held fixed. On
...
a residual of \MeanResidual{}. \NFailsFloor{} of \NModels{} models do not clear
their own replicate floor. The metric keeps only choice direction: conviction
collapses \MedianDecisiveRatio$\times$ on nonsense, while direction accuracy
barely moves. A discarded channel of the same forward pass separates the arms at
AUROC \DetBestAuroc{}, against \DetKeptAuroc{} for the channel coherence keeps.
Installed personas still displace real outcomes further than invented ones, so
the instrument is not blunt. The metric is not broken. It is unanchored.
\end{abstract}
