...
\begin{frame}{Result 4 --- the registered test is falsified}
  \textbf{The prediction did not clear its pre-declared threshold.} We report it
  as it came out.

  \bigskip
  \begin{itemize}
    \item The second clause of the registration is \hl{why}: cross-axis pairs
      were predicted near zero and are strongly positive. Every persona
      displaces utilities in a \emph{shared} direction, and a test that reads
      absolute signs cannot see structure underneath that common component.
    \item Read against cross-axis as the within-model control the registration
      intended, opposed pairs sit below it in \hl{every model $\times$ arm
      combination} --- and nearly as strongly on outcomes that refer to nothing.
    \item That contrast was \emph{not} the registered statistic. It is labelled
      exploratory and, at four models, is not claimed.
  \end{itemize}

  \bigskip
  One cell was excluded by the harness's own answer-mass verdict, and its model
  was dropped from \hl{both} arms --- a between-arm comparison over different
  populations is partly a comparison of populations. The verdict is unchanged
  either way, which is the only reason it is reportable as a correction.
\end{frame}
