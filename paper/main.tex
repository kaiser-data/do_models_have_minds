...
We state two things about this cell rather than pooling it. It was run at
\LlamaThreeThreeSevenZeroBInstructHostReps{} design seed, so it carries no
noise floor and we do not claim
\LlamaThreeThreeSevenZeroBInstructHostResid{} is distinguishable from zero ---
only that it is not the large positive residual a scale explanation predicts.
And it was served by a hosted API rather than our own vLLM harness, so it is
reported \emph{beside} the ladder in Figure~\ref{fig:scale} and never inside its
mean. The comparison it licenses is directional, and in that direction the
answer is that scale does not rescue the metric.
...
instrument, not of any model and not of our harness; a prefill variant would
recover them, and until one exists their numbers are not poolable with the rest
