...
\title{\textbf{Does the Persona Change the Preference,\\ or Only the Prose?}\\[.5em]
\large Replace every outcome with invented words and a model's preferences
barely move,\\ at every size we tested. The coherence number does not notice
---\\ so it cannot be evidence that the model does.}
...
\LadderHosted{}, which returns a residual of \HostedResidual{} against its own
\HostedReps-replicate noise floor of \HostedFloor{} --- it \HostedClears{} it.
Frontier
...
of \NModels{} models clearing their own replicate floor and two falling below
it. That the residual is only marginally separable from zero \emph{is} the
...
\S\ref{sec:strength} ranks all of them by how much weight they bear. The two
...
\texttt{Qwen3-235B-A22B-Instruct-2507} is \NHosted{}$\times$ the largest model
in the self-hosted ladder and 3.4$\times$ the largest previously measured here,
...
served by a hosted API rather than our own vLLM harness, which is a different
...
\section{How much to believe each of these}\label{sec:strength}
...
so three do not, and two are negative.
...
The title asks whether an installed persona changes what a model wants or only
...
effect falls from $+0.517$ to $+0.258$, and the share of conditions below the
diagonal falls from 90\% to 75\%. Roughly half of what looks like ``the model
...
\midrule
\texttt{results/} & 212 & 1{,}051{,}200 & self-hosted vLLM on rented L4/A10G (Modal) \\
...
the repository. \textbf{The raw cell archive is not yet public}: it lives on a
private Modal volume, so third-party reproduction \emph{from raw outputs} is
not currently possible --- only verification, once a copy is obtained.
Publishing it as a release or Zenodo deposit is the outstanding fix, and at
\CorpusGB\,GB it is within the size limits of either. Everything downstream of
the raw cells --- \texttt{card.json}, the figures, the tables, every macro in
this paper --- rebuilds from a clean clone once \texttt{results/} is present.
