...
involving these models as carrying an uncontrolled harness difference. The fix
is not expensive --- render, hash and diff the templated input per model before
a sweep, which \texttt{scripts/render\_prompts.py} now does and which gates
future runs --- but it was not in place for the results reported here. We state
it first among the limitations because it was invisible in every artifact we
kept: each recorded field described our intent rather than the model's input,
and no result would have looked wrong.
