...
\large A missing control for emergent value systems in language models}
...
\begin{abstract}
\noindent
Utility Engineering \citep{mazeika2025utility} fits a Thurstonian utility model
to a language model's pairwise choices and reports its held-out accuracy as
\emph{structural coherence}, finding that this quantity rises with scale and
concluding that coherent value systems emerge in large language models. Its
robustness checks vary how the question is asked --- language, syntax, framing,
option labels, context --- but none varies whether the outcomes being ranked
\emph{mean anything}. We build that control: the same 510 outcomes with their
referents replaced by invented words, holding the prompt, the pair set, the fit
and the metric fixed.

Our central finding concerns scale. Across one model family from \LadderSmall{}
to \LadderLarge{}, real-outcome coherence rises \LadderRRise{} while coherence
on \emph{meaningless} outcomes rises \LadderNRise{} --- roughly six times as
fast --- so the content-attributable residual \emph{falls} by
...
aggregate: across \NModels{} models, \NSplits{} evaluation splits and three
independent design replicates each (\NCells{} cells), real outcomes score
\MeanR{} and meaningless ones \MeanFloor{} (residual \MeanResidual{}), because
the metric thresholds preferences to hard labels and so records direction while
...
preference structure is detectable when present --- and inducible. We do not
...
by meaning and takes the entropy of the clusters; semantic entropy probes
approximate that from hidden states with no sampling at all; semantic energy
reads it off penultimate-layer logits on the argument that post-softmax
probabilities discard the model's inherent confidence.
