...
\large A referent-free control for value-coherence metrics:\\
invented outcomes score \MeanFloor{}, and character count
explains \SurfNmRsq{} of the ranking}
...
circumplex, used as an externally authored criterion for whether an installed
...
\LadderHosted{}, which returns a residual of \HostedResidual{}. Frontier
...
cells, \NSplits{} splits, three design replicates each); a linear model over
seven surface features recovers \SurfNmRsq{} of the ranking of invented
outcomes against \SurfRRsq{} of the real ranking, and removing that span halves
the agreement between models on the invented arm (cross-model $r$
\SurfNmCorrRaw{} to \SurfNmCorrCtrl). Put real and invented outcomes in direct
competition and preference for the real one runs \MixPreferRealLo{} to
\MixPreferRealHi{} across models --- above indifference everywhere, yet
\MixNBelowHalf{} of \MixNPairsTotal{} individual pairs still fall the other way:
under identifiable conditions these models prefer an outcome that denotes
nothing to one that does. The instrument is not merely blunt, because an
installed persona displaces real outcomes far more than invented ones in
...
first, and carry the length gap as a caveat.
...
The data are unambiguous. Across all \AvoidNModels{} models, the rank
correlation between a real outcome's fitted utility and how often it beats
gibberish runs \AvoidCorrLo{} to \AvoidCorrHi{}. The real outcomes that lose sit
at the \AvoidPctileLo--\AvoidPctileHi th percentile of the utility
distribution --- the bottom fifth, and for several models the bottom tenth.
Gibberish does not beat arbitrary outcomes. It beats the worst ones.

How often it happens is a property of the model, not of the battery: the
gibberish win rate runs \AvoidWinRateLo\% to \AvoidWinRateHi\% across models,
with \AvoidWinRateHiModel{} at the top. By category the losses concentrate in
\AvoidTopCategory{} (\AvoidTopRate\% of its pairs), and independent models agree
on that ranking at \AvoidConcordance{} --- moderate, and stated here because
one design seed cannot support a category claim on its own. We report the
category pattern as suggestive; the utility relationship above does not depend
on it.

\paragraph{Why the reassuring reading is the worrying one.} Avoidance is better
news about comprehension and worse news about safety. A system that scores an
unparseable string \emph{above} a bad outcome has a default sink: whenever the
real option can be made to look sufficiently bad, the meaningless option wins.
An adversary who cannot change what a model wants may still be able to change
what it picks, by degrading the alternatives rather than by promoting a target
--- and nothing computed inside a single arm can see this, because within an
arm the sink is not on the ballot. This also sharpens the paper's title: the
persona result says an installed trait moves preferences about half as much as
it appears to (\S\ref{sec:persona}), while this section says the preferences it
moves are real enough to be exploited.

\paragraph{What we did not establish.} Whether a risk-averse persona
\emph{widens} the sink --- whether telling a model to be cautious makes it take
the nonsense option more often --- is the obvious next experiment and we did not
run it. It needs MIXED cells under the persona conditions, which do not exist:
the MIXED arm was run only at baseline. That is a two-hour sweep on the
existing harness and it would convert this section's safety argument from an
...
design separates that from meaning.} \armNm{} runs \BatCharGapPct\% longer in
characters than \armR{} (words match to \BatWordGapPct\%), and
