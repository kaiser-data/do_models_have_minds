...
\paragraph{Scope, and a reversal.} \NeutNModels{} models, two arms each; one
further model (Qwen3.5-9B) is still incomplete and is reported as such rather
than dropped. We ran this control first on a single model, and on that model
concluded plainly that the floor survives. It does --- for that model, which
turns out to be one of the two that still engages with the invented arm. At
